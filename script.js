...
const createCard = (task, type = "sports", description = "Nenhuma descrição", status="not-completed", index) => {
...
const renderList = () => {
    ...
    banco.forEach((card, index) => createCard(card.task, card.type, card.description, card.status, index));
    setBanco(banco);
}
...
const takeTaskAndDesc = (index) =>{
    ...
    banco.forEach((banco, indexArray )=>{
        if(index == indexArray){
            console.log("o tipo é "+banco.type)
            ...
            check.forEach(check =>{
                if(check.value == typeEdit){
                    check.checked = true;
                    console.log("meu check "+ check.value);
                }
            })
        }
    })
...
const inputEdits = () =>{
    ...
    const flag = verifyInputs(value, input);
    if(flag){
        console.log("indice salvo foi "+ savedIndex)
        const banco = getBanco();
...
const clickItem = (evento) => {
    ...
    if(element.classList == 'confirm-task'){
        ...
        console.log("ENTROU AQUI NO CLIK    ")
        updateItem(index);
...
const updateItem = (index) =>{
    const banco = getBanco();
    banco[index].status = banco[index].status === "completed-task" ? "not-completed" : "completed-task";
    setBanco(banco);
    renderList();
}
